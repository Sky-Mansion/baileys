 ...
 * so it can be retried by the caller, without masking fatal unrecoverable disconnects
 * */
function mapWebSocketError(handler) {
    return (error) => {
        const isFatal = error?.message?.includes('fatal') || error?.code === 'ECONNREFUSED';
        const statusCode = isFatal ? DisconnectReason.connectionClosed : getCodeFromWSError(error);

        const boomError = new Boom(`WebSocket Error (${error?.message})`, {
            statusCode,
            data: error,
            isFatal: isFatal
        });
