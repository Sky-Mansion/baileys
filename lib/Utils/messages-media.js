    ...
    while (attempt < maxRetries) {
        try {
            ...
            if (attempt >= maxRetries) {
                if (opts.logger) opts.logger.error(`Media download failed after ${maxRetries} attempts: ${error.message}`);
                throw error;
            }
            const delayMs = Math.pow(2, attempt) * 1000;
            if (opts.logger) opts.logger.warn(`Media download failed: ${error.message}. Retrying in ${delayMs}ms...`);
